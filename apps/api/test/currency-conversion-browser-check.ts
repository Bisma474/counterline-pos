...
// End-to-end verification that changing a store's currency in StoreDetails.tsx actually
// converts every product's price by a real exchange rate — not just relabels the same integer
// cents as a different currency (which would silently turn $12.50 into "12.50 PKR", off by
// roughly 278x). Hits the real open.er-api.com FX service (no key required) since that's
// exactly what's being integrated; the assertion re-fetches the same rate independently rather
// than trusting whatever the server used, to prove the math, not just that a number changed.

const root = fileURLToPath(new URL('../../../', import.meta.url))
const pictures = root + 'docs/screenshots/currency-conversion/'
...
// One product at a known price ($12.50) so the converted amount can be checked exactly.
...
identity.get('/rest/v1/stores', (_req, res) => { res.json([{ id: store, name: 'Fixture Currency Store' }]) })
...
try {
  ...
  // Fetch the same rate independently, right before triggering the conversion, so the assertion
  // proves the actual math rather than trusting whatever number the server produced.
  const rateResponse = await fetch('https://open.er-api.com/v6/latest/USD')
  const rateData = (await rateResponse.json()) as { rates: Record<string, number> }
  const expectedRate = rateData.rates.PKR
  assert.ok(Number.isFinite(expectedRate) && expectedRate > 0, 'PKR rate should be available from the FX service')
  const expectedCents = Math.round(1250 * expectedRate)
  ...
  assert.equal(response.status(), 200, 'PATCH /stores/:id should succeed')
  const body = await response.json() as { repriced?: { product_count: number; rate: number }; currency: string }
  assert.equal(body.currency, 'PKR')
  // The stores insert trigger auto-seeds 8 demo products; plus the one added below, every
  // product in the store must be repriced, not just the one this test specifically checks.
  assert.equal(body.repriced?.product_count, 9, 'Every product in the store should be repriced, demo-seeded ones included')
  await expect(page.getByText('repriced', { exact: false })).toBeVisible()

  // 1. Verify the server actually persisted a converted price, not the original $12.50 relabeled.
  ...
  const actualCents = Number(productRow.rows[0].unit_price_cents)
  assert.notEqual(actualCents, 1250, 'The price must not stay as the raw old-currency integer after conversion')
  // Allow a tiny tolerance since the rate can tick over between our independent fetch and the
  // server's own fetch (rates update roughly every 24h, so this is only a safety margin).
  assert.ok(Math.abs(actualCents - expectedCents) <= 5, `Expected close to ${expectedCents} PKR-cents, got ${actualCents}`)

  // 2. Verify the local Dexie cache was updated too — /products should show the converted price.
  //    Check the numeric amount only (not the currency symbol/prefix): Node and Chromium can
  //    render Intl.NumberFormat's PKR symbol differently depending on bundled ICU data — a
  //    pre-existing formatCents() quirk for less-common currencies, unrelated to this fix.
  await page.goto('http://127.0.0.1:3196/products')
  await expect(page.getByText('Test Mug')).toBeVisible()
  const expectedAmount = (actualCents / 100).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
  await expect(page.getByText(expectedAmount, { exact: false })).toBeVisible()
  await page.screenshot({ path: `${pictures}products-after-conversion-1440.png`, fullPage: true })

  console.log(`PASS: currency conversion USD->PKR at rate ${expectedRate} converted $12.50 to ${actualCents} cents (~${(actualCents / 100).toFixed(2)} PKR), persisted server-side and mirrored into the local cache.`)
} finally {
