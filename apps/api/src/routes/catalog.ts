...
async function snapshot(req: import('express').Request, res: import('express').Response, terminal = false) {
  try {
    ...
    try {
      ...
      const products = await client.query('select id,store_id,sku,barcode,name,category_id,tax_rate_id,unit_price_cents::text,active,revision::text,image_url,low_stock_threshold from public.pos_products where store_id = $1 order by name', [storeId])
