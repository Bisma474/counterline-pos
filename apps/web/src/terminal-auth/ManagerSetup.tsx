...
export function ManagerSetup({ screen }: { screen: 'terminals' | 'employees' }) {
  ...
  const [roleByStore, setRoleByStore] = useState<Map<string, string>>(new Map())
  const isOwner = roleByStore.get(storeId) === 'owner'
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const { data, error: membershipError } = await client.from('store_memberships').select('store_id, role').eq('user_id', user.id).eq('active', true).in('role', ['owner', 'manager'])
        ...
        if (active) { setStores(result.data); setStoreId(result.data[0]?.id ?? ''); setRoleByStore(new Map(data.map(row => [row.store_id, row.role]))) }
