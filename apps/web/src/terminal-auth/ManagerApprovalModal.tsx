...
export function ManagerApprovalModal({ cache, reason, title = 'Authorize this discount', submitLabel = 'Approve discount', onApprove, onClose }: {
  cache: TerminalCache
  reason: string
  title?: string
  submitLabel?: string
  ...
  return <div className="manager-approval-overlay" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <section ref={dialog} className="manager-approval-dialog cashier-card" role="dialog" aria-modal="true" aria-labelledby="manager-approval-title">
      ...
      {managers.length > 0 && <>
        ...
        <button className="cta cashier-unlock-button" type="button" disabled={!canSubmit} onClick={() => void submit()}>{busy ? 'Verifying…' : submitLabel}</button>
      </>}
    </section>
  </div>
}
