...
export function PaymentScreen({ terminal = false }: { terminal?: boolean }) {
  ...
  return <section className="payment-page"><div className="pay-main"><Link to={terminal ? '/pos/register' : '/register'}>← Back to sale</Link><p className="kicker">PAYMENT</p>
  ...
  </div><aside className="payment-summary"><div className="change"><small>Change due</small><b>{formatCents(change, currency)}</b></div>
    ...
    <button className="cta" type="button" disabled={!canComplete} onClick={() => void submit()}>{busy ? 'Saving sale…' : 'Complete sale'}</button>
