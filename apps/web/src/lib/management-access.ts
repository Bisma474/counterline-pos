...
export interface FinancialAccess { storeId: string; role: 'owner' }
...
export async function resolveFinancialAccess(): Promise<FinancialAccess> {
  ...
  if (!navigator.onLine) {
    ...
    if (parsed.role !== 'owner' || Date.now() - Date.parse(parsed.validatedAt) >= 7 * 24 * 60 * 60 * 1000) {
      throw new Error('Offline reporting authorization has expired. Connect to validate access.')
    }
    return { storeId: parsed.storeId, role: 'owner' }
  }
  ...
  if (data?.[0]?.role !== 'owner') {
    ...
    throw new Error('Detailed financial reporting is currently available to store owners only.')
  }
  await posDb.sync_metadata.put({ key, value: JSON.stringify({ storeId, role: 'owner', validatedAt: new Date().toISOString() }) })
  return { storeId, role: 'owner' }
}
