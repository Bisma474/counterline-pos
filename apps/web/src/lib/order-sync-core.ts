...
export type SyncState = 'pending' | 'in_flight' | 'blocked' | 'rejected' | 'needs_signin' | 'synced'
...
export function classifySyncState(entry: Pick<OutboxEntry, 'status' | 'failure_kind' | 'lease_owner' | 'lease_expires_at'>, now = Date.now()): SyncState {
  ...
  if (entry.failure_kind === 'authentication') return 'needs_signin'
  if (entry.failure_kind === 'validation') return 'rejected'
  return 'pending'
}
...
export const SYNC_STATE_LABELS: Record<SyncState, string> = {
  ...
  needs_signin: 'Sign in again to sync',
  synced: 'Synced',
}
