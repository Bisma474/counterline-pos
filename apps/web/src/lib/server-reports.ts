...
export interface ServerOversoldProduct { id: string; name: string; sku: string; current_stock: number }

export function fetchOversold(storeId: string): Promise<{ products: ServerOversoldProduct[] }> {
  return request<{ products: ServerOversoldProduct[] }>(`/reports/oversold?store_id=${encodeURIComponent(storeId)}`)
}
