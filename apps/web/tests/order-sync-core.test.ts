...
test('classifySyncState maps outbox fields to the six FEAT-STAT-02 states', () => {
  ...
  assert.equal(classifySyncState({ ...base, status: 'failed', failure_kind: 'authentication' }), 'needs_signin')
})
...
// retrying everything except a synced entry or a genuine server-side 'validation' rejection.
// 'authentication' has its own 'needs_signin' SyncState distinct from 'rejected', so a naive
