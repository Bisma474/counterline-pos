...
## 7. Five-day MVP scope control

The sprint goal is a demonstrable core flow:

```text
Owner creates store → manager signs in → catalog loads → cashier adds items →
cash payment completes → order appears in history → mobile and laptop layout work
```

Do not start these features unless the team lead explicitly assigns them after the core flow passes:

- Full offline synchronization and device provisioning
- Offline cashier PIN authentication
- Refunds
- Customer merge or loyalty system
- WhatsApp automation
- Real card-terminal integration
- LAN relay
- Advanced reports, shifts, or inventory adjustment tools
